...
if (mysqli_num_rows($selusers) > 0) :
    ...
    if (!empty($fecha_inicio) && !empty($fecha_fin)) {
        $consulta = "SELECT SQL_CALC_FOUND_ROWS * FROM retiro WHERE fecha_retiro BETWEEN '$fecha_inicio' AND '$fecha_fin' LIMIT $inicio, $regpagina";
        $totalregistros = mysqli_query($mysqli, "SELECT FOUND_ROWS()");
    } else {
        $consulta = "SELECT SQL_CALC_FOUND_ROWS * FROM retiro LIMIT $inicio, $regpagina";
        $totalregistros = mysqli_query($mysqli, "SELECT FOUND_ROWS()");
    }
        ...
        <div class="row">
            <div class="col-md-12 text-center">
                <form class="form-horizontal" method="POST" action="">
                    <fieldset>
                        <legend>"Visualiza y descarga tu informe de retiros."</legend>
                        ...
                        <div class="form-group">
                            <div class="col-lg-12">
                                ...
                                <a href="./lib/pdf_retiros.php" class="btn btn-sm btn-success" target="_blank"><i class="fa fa-print" aria-hidden="true"></i> Generar PDF</a>
                            </div>
                        </div>
                    </fieldset>
                </form>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <?php
                                        $formatted_date = date('d/m/Y', strtotime($row['fecha_retiro']));
                                        $formatted_time = date('h:i:s a', strtotime($row['fecha_retiro']));
                    ...
                    <?php if ($numeropaginas >= 1) : ?>
                        <nav aria-label="Page navigation" class="text-center">
                            <ul class="pagination">
                                ...
                                <?php else : ?>
                                    <li>
                                        <a href="./index.php?view=HistorialRetiros&pagina=<?php echo $pagina - 1; ?>" aria-label="Previous">
                                            <span aria-hidden="true">&laquo;</span>
                                        </a>
                                    </li>
                                <?php endif; ?>
                                ...
                                for ($i = 1; $i <= $numeropaginas; $i++) {
                                    if ($pagina == $i) {
                                        echo '<li class="active"><a href="./index.php?view=HistorialRetiros&pagina=' . $i . '">' . $i . '</a></li>';
                                    } else {
                                        echo '<li><a href="./index.php?view=HistorialRetiros&pagina=' . $i . '">' . $i . '</a></li>';
                                    }
                                }
                                ?>
